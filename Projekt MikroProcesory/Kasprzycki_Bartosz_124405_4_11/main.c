...
uint8_t current_accel_range = 0;
uint8_t current_gyro_range = 0;
...
uint32_t GetTimeElapsed(uint32_t start_tick);
...
void MPU6500_Maintain(void) {
    ...
    switch (mpu_init_state) {
        ...
        case MPU_INIT_STATE_WAIT_PWR:
        	...
        	if (GetTimeElapsed(mpu_timer) > 100) {
        ...
        case MPU_INIT_STATE_WAKEUP:
        	if (i2c_busy == false) {
        		...
        		if (HAL_I2C_Mem_Write_IT(&hi2c1, MPU_ADDR, PWR_MGMT_1_REG, 1, &i2c_data, 1) != HAL_OK) {
        			i2c_busy = false;
        		} else {
        			mpu_timer = HAL_GetTick();
        ...
        case MPU_INIT_STATE_WAIT_CONFIG:
        	...
        	if (GetTimeElapsed(mpu_timer) > 10) {
...
void MPU6500_StartRead(void) {
    if (i2c_busy == false) {
        ...
        // Ustawiamy flagę PRZED startem transmisji
        i2c_busy = true;

		if(HAL_I2C_Mem_Read_IT(&hi2c1, MPU_ADDR, reg, 1, mpu_raw_data, 14) != HAL_OK) {
            i2c_busy = false;
        }
	}
}
...
// Wywoływane, gdy zakończy się ODCZYT (Read) z czujnika
// UWAGA: Ta funkcja zastępuje/rozszerza Twoją obecną HAL_I2C_MemRxCpltCallback
void HAL_I2C_MemRxCpltCallback(I2C_HandleTypeDef *hi2c){
	if (hi2c->Instance == I2C1){
        i2c_busy = false; // ODCZYT ZAKOŃCZONY - zwalniamy magistralę

		// --- Tutaj Twój stary kod przetwarzania danych ---
		MeasurementData *m = &history_buffer[head_idx];

		m->seq_num = current_seq++;
		m->timestamp = HAL_GetTick();

		// Konwersja surowych danych
...
// Wywoływane, gdy wystąpi BŁĄD sprzętowy I2C
...
void FSM_ProcessByte(uint8_t byte){
	...
	if (parser.index >= (RX_BUFFER_SIZE - 1)) {
	...
	switch (parser.state){
		...
		case STATE_ADDR:
            // Adres to 4 znaki ASCII (tutaj sprawdzamy zakres A-Z, można dostosować)
		...
		case STATE_DATA:
            ...
            else if (!IsHexChar(byte)) {
				parser.state = STATE_WAIT_SOF;
				parser.index = 0;
			}
            else {
                if ((parser.index - FRAME_HEADER_LEN) < MAX_DATA_LENGTH){
					parser.buffer[parser.index++] = byte;
				} else {
					parser.state = STATE_WAIT_SOF;
					parser.index = 0;
...
void SendResponse(const char* cmd_code, const char* payload){
	...
	uint16_t crc = CalcCRC16(frame_buf, strlen(frame_buf));
...
void ProcessCommand(char* frame, int len){
	...
	// Walidacja czy addres wysyłającego nie jest ten sam co naszego urządzenia
	if(strncmp(&frame[1], MY_DEV_ID, 2) == 0) {
		return;
	}
	...
	// Weryfikacja CRC
	int content_len = sep_ptr - &frame[1];
	uint16_t calced_crc = CalcCRC16(&frame[1], content_len);

	char crc_str_rec[5];
	strncpy(crc_str_rec, sep_ptr + 1, 4);
	crc_str_rec[4] = 0;

	if (!IsValidHex(crc_str_rec, 4)){
		SendResponse("E1", "01");
		return;
	}
	...
	switch (cmd) {
        ...
        {
        	if (data_len !=8 || !IsValidHex(data_ptr, 8)){
        		SendResponse("E1", "03");
        		break;
        	}
		...
		case 0x30:
			if (data_len != 4 || !IsValidHex(data_ptr, 4)){
				SendResponse("E1", "03");
				break;
			}
			uint8_t args[2];
			HexToBin(data_ptr, args, 2);

			uint8_t sensor_type = args[0];
			uint8_t range_idx = args[1];
			...
			uint8_t reg_addr = 0;
			uint8_t reg_val = (range_idx << 3);

			if (sensor_type == 0x00) {
				reg_addr = ACCEL_CONFIG_REG;
				current_accel_range = range_idx;
			} else if (sensor_type == 0x01) {
				reg_addr = GYRO_CONFIG_REG;
				current_gyro_range = range_idx;
			} else {
				SendResponse("E1", "03");
				break;
			}
			...
			if (i2c_busy == false) {
				static uint8_t i2c_conf_buf;
				i2c_conf_buf = reg_val;

				i2c_busy = true;

				if(HAL_I2C_Mem_Write_IT(&hi2c1, MPU_ADDR, reg_addr, 1, &i2c_conf_buf, 1) == HAL_OK) {
					SendResponse("E0", "30"); // ACK
				} else {
					i2c_busy = false;
					SendResponse("E1", "04");
				}
		...
		case 0x31:
			if (data_len != 2 || !IsValidHex(data_ptr, 2)){
				SendResponse("E1", "03");
				break;
			}

			uint8_t type_arg[1];
			HexToBin(data_ptr, type_arg, 1);

			uint8_t current_val = 0;

			if (type_arg[0] == 0x00) {
				current_val = current_accel_range;
			} else if (type_arg[0] == 0x01) {
				current_val = current_gyro_range;
			} else {
				SendResponse("E1", "03");
				break;
			}
			...
			char resp_hex[3];
			sprintf(resp_hex, "%02X", current_val);
			char payload[10];
			sprintf(payload, "31%s", resp_hex);
			SendResponse("E0", payload);
			break;
...
int main(void)
{
  ...
  while (1)
  {
      ...
      if (acquisition_running && (mpu_init_state == MPU_INIT_STATE_IDLE)) {
    	  if (GetTimeElapsed(last_acq_time) >= acquisition_interval) {
    	      last_acq_time = HAL_GetTick();
    	      MPU6500_StartRead();
    	  }
      }
